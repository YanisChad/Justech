...
async function fetchKeyboard() {
    ...
    const data = await page.evaluate(() => {
        let data = [];
        const elements = document.querySelectorAll('#app > div > div.product-list > div.product-list__main-wrapper > main > div.vue-recycle-scroller.product-list__products.ready.page-mode.direction-vertical > div.vue-recycle-scroller__item-wrapper > div:nth-child(1) > div > a > div');
        for (const element of elements){
            data.push({
                name: element.querySelector('div.pl-product__content > div.pl-product__sublabel').textContent.trim(),
            });
        }
        return data;
    });
